...
export type LaunchSafetyCoverage = {
  category: 'structural' | 'restraint' | 'adas' | 'ev_hv' | 'property_code' | 'other';
  source: string;
  regions: string[];
  approved: boolean;
};
...
export type LaunchManifest = {
  ...
  market: string;
  assetClasses: string[];
  dataRights: LaunchDataRight[];
  safetyCoverage: LaunchSafetyCoverage[];
  privacyReviewReference: string;
  privacyApproved: boolean;
  securityReviewReference: string;
  securityApproved: boolean;
  pilotEvidenceReference: string;
  pilotValidated: boolean;
  backupRestoreEvidenceReference: string;
  rpoMinutes: number;
  rtoMinutes: number;
};
...
export function evaluateLaunchReadiness(manifest: LaunchManifest, env: NodeJS.ProcessEnv = process.env): LaunchReadiness {
  ...
  if (manifest.version !== 1) block('manifest', 'unsupported launch manifest version');
  ...
  for (const name of ['R2_ACCOUNT_ID','R2_BUCKET','R2_ACCESS_KEY_ID','R2_SECRET_ACCESS_KEY'] as const) {
    if (!configured(env[name])) block('evidence_storage', `${name} is required`);
  }
  ...
  if (!configured(env.ELITE_OUTBOX_MAX_PENDING) || !configured(env.ELITE_OUTBOX_MAX_AGE_SECONDS) || !configured(env.ELITE_OUTBOX_MAX_EXHAUSTED)) {
    block('claims_integration', 'production outbox health thresholds must be configured');
  }
  ...
  for (const coverage of manifest.safetyCoverage) {
    if (coverage.approved && coverage.source.trim() && coverage.regions.length > 0) requiredSafety.delete(coverage.category);
  }
...
export async function loadLaunchManifest(path: string): Promise<LaunchManifest> {
  const parsed = JSON.parse(await readFile(path, 'utf8')) as Partial<LaunchManifest>;
  if (parsed.version !== 1) throw new Error('invalid_launch_manifest_version');
  return parsed as LaunchManifest;
}
