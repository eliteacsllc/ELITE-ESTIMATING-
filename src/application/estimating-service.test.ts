...
function approvedLine(): EstimateLine {
  return {
    ...
    partOrMaterial: { amountMinor: 50000, currency: 'USD' }, laborHours: 2, laborRate: { amountMinor: 7500, currency: 'USD' },
    total: { amountMinor: 0, currency: 'USD' }, humanApproved: true,
...
test('tenant-scoped service creates and calculates an estimate', async () => {
  ...
  const estimate = await service.create(estimator, { tenantId: 'tenant-a', asset: { assetClass: 'passenger_vehicle', vin: '1TEST' }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' });
...
test('cross-tenant creation is denied', async () => {
  const service = new EstimatingService(new InMemoryEstimateRepository());
  await assert.rejects(() => service.create(estimator, { tenantId: 'tenant-b', asset: { assetClass: 'contents' }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' }), /cross_tenant_access_denied/);
});

test('unapproved AI or human lines cannot be approved', async () => {
  const service = new EstimatingService(new InMemoryEstimateRepository());
  const estimate = await service.create(estimator, { tenantId: 'tenant-a', asset: { assetClass: 'passenger_vehicle' }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' });
...
test('MOTOR-based estimate requires repair plan before approval', async () => {
  const service = new EstimatingService(new InMemoryEstimateRepository());
  const estimate = await service.create(estimator, { tenantId: 'tenant-a', asset: { assetClass: 'passenger_vehicle', year: 2026 }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' });
  const line: EstimateLine = { ...approvedLine(), provenance: [{ provider: 'MOTOR Information Systems', retrievedAt: new Date().toISOString(), licenseClass: 'licensed' }], guide: { source: 'motor_gte', partBasis: 'new_oem', workTimeBasis: 'database' } };
  await service.replaceLines(estimator, estimate.id, [line]);
...
test('incomplete repair plan blocks safety-critical approval', async () => {
  const service = new EstimatingService(new InMemoryEstimateRepository());
  const estimate = await service.create(estimator, { tenantId: 'tenant-a', asset: { assetClass: 'passenger_vehicle' }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' });
  const line: EstimateLine = { ...approvedLine(), safetyCritical: true, procedureRefs: ['OEM:repair-procedure'] };
  await service.replaceLines(estimator, estimate.id, [line]);
...
test('attached domain workflow becomes an approval gate', async () => {
  const service = new EstimatingService(new InMemoryEstimateRepository());
  const estimate = await service.create(estimator, { tenantId: 'tenant-a', asset: { assetClass: 'contents' }, locale: 'en-US', currency: 'USD', jurisdiction: 'US' });
  await service.replaceLines(estimator, estimate.id, [approvedLine()]);
  let workflow = createDomainWorkflow({ assetClass: 'contents' }, 'contents');
  await service.replaceDomainWorkflow(estimator, estimate.id, workflow);
  await assert.rejects(() => service.approve(reviewer, estimate.id), /domain_workflow_incomplete/);
  for (const step of workflow.steps) workflow = updateDomainWorkflowStep(workflow, { stepId: step.id, status: 'complete', completedBy: 'u1', evidenceRefs: [`e:${step.id}`] });
  await service.replaceDomainWorkflow(estimator, estimate.id, workflow);
  const approved = await service.approve(reviewer, estimate.id);
  assert.equal(approved.status, 'approved');
});
