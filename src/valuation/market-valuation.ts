...
export function calculateMarketValuation(input: {
  ...
  const evidence = [
    ...selected.map((c) => ({
      ...
      ...(c.retrievedAt !== undefined ? {retrievedAt:c.retrievedAt} : {}),
      value: c.adjustedPrice,
      ...(c.sourceEvidenceHash !== undefined ? {evidenceHash:c.sourceEvidenceHash} : {}),
      ...(c.sourceSnapshotKey !== undefined ? {snapshotKey:c.sourceSnapshotKey} : {}),
      ...(c.sourceSnapshotMimeType !== undefined ? {snapshotMimeType:c.sourceSnapshotMimeType} : {}),
    })),
    ...(input.bookSources ?? []).map((s) => ({
      ...
      value: round(s.value),
      ...(s.evidenceHash !== undefined ? {evidenceHash:s.evidenceHash} : {}),
      ...(s.licenseRef !== undefined ? {licenseRef:s.licenseRef} : {}),
    })),
  ];
